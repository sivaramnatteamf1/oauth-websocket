(function() {
  ...
  function $WebSocketProvider($rootScope, $q, $timeout, $websocketBackend, $cookieStore) {
    ...
    $WebSocket.prototype._onCloseHandler = function _onCloseHandler(event) {
      ...
      if ((this.reconnectIfNotNormalClose && event.code !== this._normalCloseCode) || this._reconnectableStatusCodes.indexOf(event.code) > -1) {
        ...
        // If cookie token not available in browser do not reconnect to avoid conencts in logout case
        // For some reason reconnects are in queue or websocket is not closed due to client issues when they are actually logged out
        if ( typeof $cookieStore.get("token") != "undefined" && $cookieStore.get("token").access_token != "undefined") {
          this.reconnect();
        }
      }
    };
    ...
    $WebSocket.prototype.reconnect = function reconnect() {
      ...
      // Make same time for reconnect otherwise if client is not reaching and reconnect attempts are many it takes more time to connect back
      var backoffDelay = this._getBackoffDelay(2);
  ...
  .factory('$websocket', ['$rootScope', '$q', '$timeout', '$websocketBackend','$cookieStore' , $WebSocketProvider])
  .factory('WebSocket',  ['$rootScope', '$q', '$timeout', 'WebsocketBackend', '$cookieStore' ,  $WebSocketProvider])
