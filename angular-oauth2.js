...
(function(root, factory) {
    ...
    function OAuthProvider() {
        ...
        this.$get = function($http, OAuthToken, base64) {
            var OAuth = function() {
                function OAuth() {
                    if (!config) {
        ...
        this.$get.$inject = [ "$http", "OAuthToken", "$base64" ];
    }
    var _prototypeProperties = function(child, staticProps, instanceProps) {
        if (staticProps) Object.defineProperties(child, staticProps);
        if (instanceProps) Object.defineProperties(child.prototype, instanceProps);
                ...
                _prototypeProperties(OAuthToken, null, {
                    setToken: {
                        value: function setToken(data) {
                            /*Set the data with createdTimeStamp*/
                            data.createdTimeStamp = new Date().getTime()
                            return $cookieStore.put(config.name, data);
                        },
                        writable: true,
                        enumerable: true,
                        configurable: true
